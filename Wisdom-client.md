...
&#160;&#160;&#160;&#160;&#160;&#160;This kind of error, can not detect the fast synchronization fast_sync.zip compressed package, execute the script tool again.
...
|Parameter Item| Parameter Description|Effect
|---|---|---
|<div style="width:155pt">DATA_SOURCE_URL</div> | <div style="width:70pt">Database configuration</div>|The default value is```jdbc:postgresql://wdc_pgsql:5432/postgres```，docker containers are used for interconnection without modification. If you need to modify, make sure that the host name in the URL is pgsql container name, and the port is the same as the database port inside the pgsql container.
|DB_USERNAME|Database name|Default to replica
|DB_PASSWORD|Database password|Default to replica
|ENABLE_MINING|Enable mining or not|true is enable，false is close
|WDC_MINER_COINBASE|Miner's address|Address generated by WDC
|P2P_MODE|Communication mode|grpc
|P2P_ADDRESS|Node address|The mode is wisdom://192.168.1.156:9585
|BOOTSTRAPS|Seed node|Default to wisdom://47.74.183.249:9585,wdom://47.74.216.251:9585,wisdom://47.96.67.155:9585,wisdom://47.74.86.106:9585
|MAX_BLOCKS_PER_TRANSFER|Maximum number of synchronization blocks|Default to 256
|ENABLE_DISCOVERY|Enable node discovery or not|true is enable，false is close
|FAST_SYNC_DIRECTORY|Snapshot file path (this path is the path within the container, which needs to be consistent with the mapping path under the configured volumes in wdc.yml)|/fast-sync
|DATABASE_DIRECTORY|Leveldb storage path (this path is the path within the container, which needs to be consistent with the mapping path under the configured volumes in wdc.yml)|/leveldb
